...
// it on the calls row revokes the link. The Plivo key and Plivo's expiring links never leave the
// server: we resolve a fresh link and stream the bytes through, Range included so seeking works.
import express from 'express';
import { Readable } from 'node:stream';
import { q } from '../db/pool.js';
import { recordingUrl, fetchRecording } from '../lib/recordings.js';
...
router.get('/:token.mp3', async (req, res) => {
  ...
  if (!url) return res.status(502).json({ error: 'recording not available from Plivo' });
  const up = await fetchRecording(url, req.headers.range ? { range: req.headers.range } : {});
  if (!up.ok) return res.status(502).json({ error: 'Plivo download ' + up.status });
