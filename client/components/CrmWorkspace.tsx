...
type Call = { queue?:'requested'|'auto';outcome?:string|null;disposition?:string|null;id:string;lead_name:string;phone:string;tse_name:string;status:string;created_at:string;answered_at:string|null;ended_at:string|null;bill_seconds:number|null;cost_usd:string|null;recording_id:string|null;hangup_cause:string|null };
...
export default function CrmWorkspace({me,onLogout}:{me:Me;onLogout:()=>void}) {
  ...
  const callTable=(preview=false)=><section className="panel crm-report-panel" aria-label="Call history">
    ...
    <div className="ad-scroll"><table className="ad-tbl crm-table"><thead><tr><th>Date (IST)</th>{admin&&<th>TSE</th>}<th>Lead</th><th>Phone</th><th>Queue / outcome</th><th>Status</th><th>Hangup reason</th><th className="num">Seconds</th><th className="num">Cost</th><th>Recording</th></tr></thead><tbody>
      {(preview?report?.calls.slice(0,6):report?.calls)?.map(call=><tr key={call.id}><td className="crm-date">{date(call.created_at)}</td>{admin&&<td>{call.tse_name || 'TSE'}</td>}<td className="name">{call.lead_name || 'Requested lead'}</td><td className="mono crm-phone">{call.phone}</td><td>{call.queue==='auto'?'Auto Calling':'Request Leads'}{call.queue==='auto'&&<div className="crm-caption">{call.outcome?.replaceAll('_',' ') || (call.ended_at?'Outcome pending':'In progress')}</div>}</td><td><Status value={call.status}/></td><td className="crm-cause">{call.hangup_cause?.replaceAll('_',' ').toLowerCase() || '\u2014'}</td><td className="num">{call.bill_seconds ?? '\u2014'}</td><td className="num">{money(call.cost_usd)}</td><td>{call.recording_id?<a className="btn btn-mini crm-recording" href={API+'/api/crm-recordings/'+call.id} target="_blank" rel="noreferrer" aria-label={'Play recording for '+(call.lead_name || call.phone)}><Play/>Play</a>:<span className="muted">{call.ended_at?'Unavailable':'Pending'}</span>}</td></tr>)}
      {!report?.calls.length&&<tr><td colSpan={admin?10:9}><div className="crm-empty"><Phone/><strong>{loading?'Loading calls...':'No calls in this period'}</strong><span>{loading?'Getting your latest calling activity.':'Calls placed through CRM will appear here. Try a different date or TSE filter.'}</span></div></td></tr>}
    </tbody></table></div>
