...
export default function TimeBar({ offset, timezone, showTime = true }: { offset: string | number | null | undefined; timezone?: string | null; showTime?: boolean }) {
  const [now, setNow] = useState(() => new Date());
  useEffect(() => { const t = setInterval(() => setNow(new Date()), 60000); return () => clearInterval(t); }, []);
  const lt = localTime(offset, now, timezone);
